...
################################################ run this with $sbatch execute_RNAseq_pipeline.sbatch
# PROGRAM:
...
inputdir="/pl/active/onishimura_lab/PROJECTS/Kayla/Dissected_Intestine/01_input"

#Metadata file. This pulls the metadata path and file from the command line
metadata=$1

#This is where the ht2 files live:
hisat2path="/pl/active/onishimura_lab/PROJECTS/Kayla/Dissected_Intestine/Ce11_Index_Build/ce11"

#This is where the genome sequence lives:
genomefa="/pl/active/onishimura_lab/PROJECTS/Kayla/Dissected_Intestine/Ce11_Index_Build/ce11_wholegenome.fa"

#This is where the gtf file lives:
gtffile="/pl/active/onishimura_lab/PROJECTS/Kayla/Dissected_Intestine/Ce11_Index_Build/ce11_annotation_ensembl_to_ucsc.gtf"
...
do
    ...
    ## execute hisat2
    cmd3="hisat2 -x $hisat2path -1 $outputdir"01_fastp/"$samplename/$samplename"_trim_1.fastq" -2 $outputdir"01_fastp/"$samplename/$samplename"_trim_2.fastq" -S ${outhisat2}${samplename}.sam --summary-file ${outhisat2}${samplename}_summary.txt --no-unal -p 2"
    echo -e "\t$ $cmd3"
    time eval $cmd3
...
cmd4="featureCounts -p -T 2 -a $gtffile -o ${outfeature}counts.txt ${samfilePath[*]}"
echo -e "\t$ $cmd4"
time eval $cmd4
...
do
    ...
    cmd5="samtools view --threads 2 -bS ${outhisat2}${seqname}.sam > ${samout}${seqname}.bam"
    echo -e "\t$ ${cmd5}"
    time eval $cmd5

    
    # Samtools: sort .bam -> _sort.bam
    cmd6="samtools sort --threads 2 -o ${samout}${seqname}_sort.bam --reference $genomefa ${samout}${seqname}.bam"
    echo -e "\t$ ${cmd6}"
    time eval $cmd6
    ...
    cmd8="bamCoverage -b ${samout}${seqname}_sort.bam -o ${samout}${seqname}_sort.bw --outFileFormat bigwig -p 2 --normalizeUsing CPM --binSize 1"
    echo -e "\t$ ${cmd8}"
    time eval $cmd8
